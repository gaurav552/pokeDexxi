...
class pokeTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
           ...
           PokeFetch.fetchBaseUrl("pokemon"){
            (fetchResult) -> Void in
                                
            switch fetchResult {
            case let .success(entryPoints):
                for entry in entryPoints {
                    self.pokUrl = entry.url.absoluteString
                    PokeFetch.fetchUrl(self.pokUrl){
                    (getPokeNames) -> Void in
                        switch getPokeNames {
                        case let .success(pokeEntry):
                            for entry2 in pokeEntry {
                                self.pokemonNames.append(entry2.name!)
                            }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "pokeTable", for: indexPath)
