...
class itemTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
           ...
           PokeFetch.fetchBaseUrl("item"){
            (fetchResult) -> Void in
                                
            switch fetchResult {
            case let .success(entryPoints):
                for entry in entryPoints {
                    self.itemUrl = entry.url.absoluteString
                    PokeFetch.fetchUrl(self.itemUrl){
                    (getItemNames) -> Void in
                        switch getItemNames {
                        case let .success(pokeEntry):
                            for entry2 in pokeEntry {
                                self.itemNames.append(entry2.name!)
                            }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "itemTable", for: indexPath)
